...
export async function sendTelegramNotification({ user, schedule, success, message, telegramBotToken, telegramChatId, locationName, checkinTime }) {
    ...
    const telegramMessage = `${statusEmoji} *Check-in SKEMARAJA ${statusText}*

👤 *Nama:* ${user.name || user.nip}
📅 *Jadwal:* ${schedule?.name || 'Manual'}
🕐 *Waktu:* ${checkinTime || timeStr}
📍 *Lokasi:* ${locationName || 'N/A'}
📱 *Status:* ${schedule?.status_wfh === '1' ? 'WFH' : 'WFO'}
...
_Auto Check-in by SKEMARAJA Puppeteer_`;
    ...
    try {
        ...
        const result = await response.json();
        if (result.ok) {
            logger.info(`📨 Telegram sent for ${user.name || user.nip}`);
        } else {
            logger.warn(`📨 Telegram failed: ${result.description || 'Unknown error'}`);
        }
    } catch (error) {
...
export async function performCheckin(config) {
    ...
    try {
        ...
        logger.debug(`Browser config: headless=${headless}, slowMo=${slowMo}`);
        logger.debug(`Geolocation: lat=${latitude}, lng=${longitude}`);
        logger.debug(`Check-in config: status=${status}, shift=${shift}`);
        ...
        logger.debug('Geolocation permission granted');
        ...
        logger.debug('Geolocation coordinates set');
        ...
        logger.debug(`Target URL: ${SKEMARAJA_URL}`);
        let retries = 3;
        while (retries > 0) {
            try {
                logger.debug(`Navigation attempt ${4 - retries}/3`);
                const response = await page.goto(SKEMARAJA_URL, {
                    waitUntil: 'networkidle2',
                    timeout: 60000,
                });
                logger.debug(`Navigation response status: ${response?.status()}`);
                logger.debug(`Final URL: ${page.url()}`);
                break;
            } catch (navError) {
                retries--;
                logger.debug(`Navigation error: ${navError.message}`);
        ...
        logger.debug(`Page loaded - URL: ${initialUrl}, Title: ${initialPageTitle}`);

        // Take screenshot of initial page for debugging (if enabled)
        if (enableScreenshot) {
        ...
        // Take screenshot after geolocation (if enabled)
        if (enableScreenshot) {
        ...
        try {
            ...
            // Log page HTML for debugging (first 500 chars)
            ...
            logger.error(`🔍 Debug: Has any input: ${!!hasAnyInput}, Has form: ${!!hasForm}`);
            ...
            throw new Error(`Form login tidak ditemukan. URL: ${currentUrl}, Title: ${pageTitle}`);
        }
