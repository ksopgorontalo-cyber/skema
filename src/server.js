...
const DEFAULT_CONFIG = {
    ...
    // Screenshot (disabled by default for VPS compatibility)
...
app.post('/api/checkin', async (req, res) => {
    ...
    try {
        ...
        const result = await performCheckin({
            ...
            headless: true,
            slowMo: 50,
            enableScreenshot: config.enableScreenshot || false
        });
...
app.get('/api/telegram/status', (req, res) => {
    ...
    res.json({
        ...
        chatId: config.telegramChatId ? config.telegramChatId.substring(0, 5) + '...' : ''
    });
});

// Save Telegram config
app.post('/api/telegram/save', (req, res) => {
    const config = loadConfig();
    const { botToken, chatId, enabled } = req.body;

    if (botToken !== undefined) config.telegramBotToken = botToken;
    if (chatId !== undefined) config.telegramChatId = chatId;
    if (enabled !== undefined) config.telegramEnabled = enabled;
...
app.post('/api/telegram/test', async (req, res) => {
    const config = loadConfig();
    const botToken = req.body.botToken || config.telegramBotToken;
    const chatId = req.body.chatId || config.telegramChatId;

    if (!botToken || !chatId) {
        return res.json({ success: false, message: 'Bot Token dan Chat ID diperlukan' });
    }

    try {
        const testMessage = `🔔 *Test Notifikasi SKEMARAJA*

✅ Koneksi Telegram berhasil!

📍 Server: Puppeteer Dashboard
...
_Auto Check-in by SKEMARAJA Puppeteer_`;
        ...
        if (result.ok) {
            ...
            return res.json({ success: false, message: result.description || 'Failed to send' });
        }
    } catch (error) {
        return res.json({ success: false, message: error.message });
    }
...
<body>
    <div class="container">
        ...
        <div class="grid">
            ...
            <div class="card">
                ...
                <div id="telegramStatus" style="margin-bottom: 16px; padding: 12px; background: var(--bg); border-radius: 8px;">
                    <span id="telegramStatusIcon">⏳</span> <span id="telegramStatusText">Mengecek status...</span>
                </div>
                ...
                <div class="form-group">
                    <label>Chat ID</label>
                    <input type="text" id="telegramChatId" placeholder="Contoh: -1001234567890 atau 123456789">
                    <small style="color: var(--text-muted);">Gunakan @userinfobot atau @getidsbot untuk mendapatkan Chat ID</small>
                </div>
                
                <div class="flex-row" style="flex-wrap: wrap;">
                    <button onclick="saveTelegramConfig()" class="btn-success btn-sm">💾 Simpan</button>
                    <button onclick="testTelegram()" class="btn-primary btn-sm">📤 Test Kirim</button>
                    <button onclick="checkTelegramStatus()" class="btn-warning btn-sm">🔍 Cek Status</button>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        async function checkTelegramStatus() {
            ...
            const iconEl = document.getElementById('telegramStatusIcon');
            ...
            try {
                ...
                if (data.success) {
                    if (data.hasToken && data.hasChatId) {
                        statusEl.style.background = 'rgba(16,185,129,0.2)';
                        iconEl.textContent = '🟢';
                        textEl.innerHTML = '<strong>Configured</strong> - Chat ID: ' + data.chatId;
                    } else {
                        statusEl.style.background = 'rgba(239,68,68,0.2)';
                        iconEl.textContent = '🔴';
        ...
        async function testTelegram() {
            ...
            if (!botToken || !chatId) {
                alert('❌ Masukkan Bot Token dan Chat ID terlebih dahulu');
                return;
            }
